...
onready var WeaponP = preload("res://Weapon.tscn")
...
func move(input_vector):
	if input_vector != Vector2.ZERO: #Si hemos obtenido algun input de movimiento
		...
		velocityPlayer = input_vector * PLAYERSPEED #Asignamos velocidad y direccion
	else:
...
func get_input(attack_vector):
	if attack_vector != Vector2.ZERO and canShootPlayer: #Si hemos obtenido algun input de ataque, y no esta en cooldown (firerate)
		...
		$AttackRate.start(attackRate) #Reiniciamos contador de firerate
		canShootPlayer = false #No permitimos volver a disparar 
#Funcion de disparo
func shoot(attack_vector):
	var weapon = WeaponP.instance() #Instanciamos un nuevo arma
	owner.add_child(weapon) #La asignamos al player
	weapon.transform = $PositionWeapon.global_transform #Hacemos que inicie en su posicion de disparo
	weapon.speed = 200 #Elegimos la velocidad de proyectil
	weapon.sprite.frame = 2 #Elegimos el sprite del proyectil
	weapon.setDirection(attack_vector) #Indicamos en que direccion debe ir
